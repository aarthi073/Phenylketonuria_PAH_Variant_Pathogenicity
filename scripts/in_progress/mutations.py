...
for uid in ids:
        ...
        prot_change = record.get('protein change', 'Unknown')        
        ...
        vlist.append([variant, vname, HGVS, prot_change, mutation_type, germ, clinical, onco])
...
for i in vlist:
        ...
        if "fs" in mut:
        	...
        	deletion.append(mut)
                ...
                mis = mut
        clean_mis = mis.rpartition(")")[0]
        mut_type = i[4]
        for i in mut_type:
                var_type.append(i)
     #   mut_classification.append(clean_mis)
...
with open("../docs/label.txt", "w") as f:
         for i in mut_classification:
                f.write(i+"\n")
#NONSENSE IS EARLY STOP CODON, BUT FRAME SHIFT IS INSERTION/DELETION
with open("../docs/frameshift.txt", "w") as f:
         for i in fs:
                f.write(i+"\n")
with open("../docs/deletions.txt", "w") as f:
         for i in deletion:
                f.write(i+"\n")
with open("../docs/silent.txt", "w") as f:
         for i in sil:
                f.write(i+"\n")
with open("../docs/nonsense.txt", "w") as f:
         for i in nonsense:
                f.write(i+"\n")
